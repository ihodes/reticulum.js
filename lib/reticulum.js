    ...
    for (var idx in statesToCurrent) {
        ...
        if (state.actions && state.actions.event) {
            for (var eidx in state.actions.event) {
                ...
                if (res) return transition(fsmi, res);
            }
        }
    }
...
var executeAction = function(fsmi, action) {
    ...
    for (var idx in action) {
        ...
        partName = part[0];
        actionFn = ACTIONS[partName];
        if (!_.exists(actionFn)) throw new Error("Procedure or guard \"" + partName + "\" is undefined.");
    
        params = _.rest(part);

        // TK TODO shouldn't reify params here; should be done within actions; some params shouldn't be reified (e.g. keys for `set`)
        ...
        if (_.isString(res)) return res; // we transition, then.
        else if (res === false) return false; // computer says no. (guard says no, but eh).
    }
    return null;
};
