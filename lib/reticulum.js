...
function transition(fsm, from, to) {
    ...
    var exitingStates  = transitionStates.fromShared;
    var enteringStates = transitionStates.toShared;
...
function execute(action, fsmi, context, response, callback, next) {
    ...
    if (_.isString(procedure)) {
        ...
        fsmi.currentStateName = toStateName;
        return executeActions(actions, context, fsmi, response, callback);
    } else {
