...
vows.describe('Validating a FSM').addBatch({
    ...
    'when validating a FSM with a bad action': {
        topic: function() {
            return fsmValidator(badfsm2);
        },

        'an error should be returned': function(topic) {
            topic.should.be.a.String;
            topic.states.states.actions.event.should.eql("event[1][0][3] ([\"if\",\"eq\",\"gotoA2\",{\"badkey\":\"isBad\"}]) must be an array (or nested array) of scalars");
         }
     }
}).export(module);
        ...
        { name: 'SubstateA3',
          ...
          states: [
              { name: 'SubsubstateA31',
                actions: {
                    event: [ [['log', 'SubsubstateA31 recieved this message']],
                             [['if', 'eq', 'gotoA2', {badkey: 'isBad'}], 'SubstateA2'] ],
                    enter: [ [['set', 'magicVar', 'testSet']],
                             [['log', 'just entered SubsubstateA31']] ],
                    exit:  [ [['log', 'just exited SubsubstateA31']] ]
                }
